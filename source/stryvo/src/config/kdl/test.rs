...
fn load_test() {
    ...
    let expected = crate::config::internal::Config {
                ...
                path_control: crate::config::internal::PathControl {
                    upstream_request_filters: vec![
                        ...
                        BTreeMap::from([
                            ...
                            ("value".to_string(), "stryvo".to_string()),
                        ]),
                    ],
        ...
        daemonize: false,
        pid_file: Some("/tmp/stryvo.pidfile".into()),
        upgrade_socket: Some("/tmp/stryvo-upgrade.sock".into()),
        upgrade: false,
    };
    ...
    for (abp, ebp) in val.basic_proxies.iter().zip(expected.basic_proxies.iter()) {
        ...
        assert_eq!(*cache_enabled, ebp.cache_enabled);
        assert_eq!(*cache_ttl, ebp.cache_ttl);
        assert_eq!(*browser_cache_enabled, ebp.browser_cache_enabled);
        assert_eq!(*browser_cache_ttl, ebp.browser_cache_ttl);
    }
