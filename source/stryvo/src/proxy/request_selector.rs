...
/// the StryvoContext.selector_buf field, using `write!` or similar formatting
/// options.
///
...
pub type RequestSelector = for<'a> fn(&'a mut StryvoContext, &'a mut Session) -> &'a [u8];

/// Null selector, useful when using "Random" or "RoundRobin" selection and this key is not used
///
/// Performs no formatting
pub fn null_selector<'a>(_ctxt: &'a mut StryvoContext, _ses: &'a mut Session) -> &'a [u8] {
    &[]
}
...
pub fn uri_path_selector<'a>(_ctxt: &'a mut StryvoContext, ses: &'a mut Session) -> &'a [u8] {
